...
def join(request, data):
    if request.method == 'POST':
        ...
        staff= Staff.objects.get(user=request.user)
        #salary_record = Staff.objects.get(id=salary_id)
        #borrow = Staff(borrow=amount)
        #borrow.save()

        staff.borrow=float(staff.borrow)+amount
        staff.dues=float(staff.dues)+amount
        staff.save()
...
def details(request, data, company_uuid):
    ...
    data['borrow'] = staff.borrow
    data['lend']=staff.lend
